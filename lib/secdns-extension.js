...
SecDnsExtension.prototype.processSecDns = function (data) {
  ...
  if (data.dsData) {
    ...
    if (data.maxSigLife) secDns["secDNS:maxSigLife"] = data.maxSigLife;
...
SecDnsExtension.prototype.updateDomainSecDnsExtension = function (data) {
  ...
  if (!(change || add || rem)) throw new Error("At least one 'chg', 'add', or 'rem' required in DNSSEC updates.");
  var actions = ["add", "rem"];
    ...
    if (actionSet) {
      ...
      if (action === 'rem' && actionSet.all !== undefined) {
        ...
        if (typeof(all) === "number") {
          all = true;
          if (all <= 0) all = false;
        } else if (typeof(all) !== "boolean") throw new Error("'all' must be a boolean or truthy number.");
  ...
  if (change) {
    var changeSecDns = {};
    if (change.maxSigLife) changeSecDns["secDNS:maxSigLife"] = change.maxSigLife;
